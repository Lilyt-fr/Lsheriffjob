...
AddEventHandler('equipementbase', function()
	...
	xPlayer.addWeapon('WEAPON_PISTOL50', Config.amountAmmo)
...
RegisterNetEvent('finalsheriff:arsenalvide')
AddEventHandler('finalsheriff:arsenalvide', function()
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
...
ESX.RegisterServerCallback('finalsheriff:getFineList', function(source, cb, category)
...
ESX.RegisterServerCallback('finalsheriff:getVehicleInfos', function(source, cb, plate)
...
ESX.RegisterServerCallback('finalsheriff:getVehicleFromPlate', function(source, cb, plate)
	MySQL.Async.fetchAll('SELECT owner FROM owned_vehicles WHERE plate = @plate', {
		['@plate'] = plate
	}, function(result)
...
ESX.RegisterServerCallback('fsheriff:getStockItems', function(source, cb)
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_sheriff', function(inventory)
		cb(inventory.items)
	end)
end)

RegisterNetEvent('fsheriff:getStockItem')
AddEventHandler('fsheriff:getStockItem', function(itemName, count)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
ESX.RegisterServerCallback('fsheriff:getPlayerInventory', function(source, cb)
...
RegisterNetEvent('fsheriff:putStockItems')
AddEventHandler('fsheriff:putStockItems', function(itemName, count)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterServerEvent('finalsheriff:spawned')
AddEventHandler('finalsheriff:spawned', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)

	if xPlayer ~= nil and xPlayer.job ~= nil and xPlayer.job.name == 'sheriff' then
		Citizen.Wait(5000)
		TriggerClientEvent('finalsheriff:updateBlip', -1)
	end
end)

RegisterServerEvent('finalsheriff:forceBlip')
AddEventHandler('finalsheriff:forceBlip', function()
	TriggerClientEvent('finalsheriff:updateBlip', -1)
end)
...
RegisterServerEvent('finalsheriff:message')
AddEventHandler('finalsheriff:message', function(target, msg)
	TriggerClientEvent('esx:showNotification', target, msg)
end)

ESX.RegisterServerCallback('finalsheriff:getArmoryWeapons', function(source, cb)
...
ESX.RegisterServerCallback('finalsheriff:addArmoryWeapon', function(source, cb, weaponName, removeWeapon)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('finalsheriff:removeArmoryWeapon', function(source, cb, weaponName)
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterServerEvent('finalsheriff:requestarrest')
AddEventHandler('finalsheriff:requestarrest', function(targetid, playerheading, playerCoords,  playerlocation)
	_source = source
	TriggerClientEvent('finalsheriff:getarrested', targetid, playerheading, playerCoords, playerlocation)
	TriggerClientEvent('finalsheriff:doarrested', _source)
end)

RegisterServerEvent('finalsheriff:requestrelease')
AddEventHandler('finalsheriff:requestrelease', function(targetid, playerheading, playerCoords,  playerlocation)
	_source = source
	TriggerClientEvent('finalsheriff:getuncuffed', targetid, playerheading, playerCoords, playerlocation)
	TriggerClientEvent('finalsheriff:douncuffing', _source)
end)
...
RegisterServerEvent('finalsheriff:handcuff')
AddEventHandler('finalsheriff:handcuff', function(target)
  TriggerClientEvent('finalsheriff:handcuff', target)
end)

RegisterServerEvent('finalsheriff:drag')
AddEventHandler('finalsheriff:drag', function(target)
  local _source = source
  TriggerClientEvent('finalsheriff:drag', target, _source)
end)

RegisterServerEvent('finalsheriff:putInVehicle')
AddEventHandler('finalsheriff:putInVehicle', function(target)
  TriggerClientEvent('finalsheriff:putInVehicle', target)
end)

RegisterServerEvent('finalsheriff:OutVehicle')
AddEventHandler('finalsheriff:OutVehicle', function(target)
    TriggerClientEvent('finalsheriff:OutVehicle', target)
end)

ESX.RegisterServerCallback('finalsheriff:getOtherPlayerData', function(source, cb, target, notify)
...
AddEventHandler("genius:sendcall", function()
	...
	for i = 1, #xPlayers, 1 do
		local thePlayer = ESX.GetPlayerFromId(xPlayers[i])
		if thePlayer.job.name == 'sheriff' then
			TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], "Central LSPD", nil, "Un Citoyen demande un agent de sheriff au commissariat", "CHAR_CALL911", 8)
		end
	end
end)
...
AddEventHandler('patron:recruter', function(societe, job2, target)
  ...
  if job2 == false then
  	...
  	TriggerClientEvent('esx:showNotification', target, "Bienvenue chez la sheriff !")
  	else
...
AddEventHandler('patron:promouvoir', function(societe, job2, target)
  ...
  if job2 == false then
  	...
  	TriggerClientEvent('esx:showNotification', target, "Vous avez été promu chez la sheriff!")
  	else
...
AddEventHandler('esx_license:addLicense', function(permis)
	local xPlayer = ESX.GetPlayerFromId(source)
	local xTarget = ESX.GetPlayerFromId(target)
...
AddEventHandler('esx_license:removeLicense', function(permis)
	local xPlayer = ESX.GetPlayerFromId(source)
  	local xTarget = ESX.GetPlayerFromId(target)
